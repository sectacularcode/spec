...
export default function CustomBuild({ userId, role } = {}) {
  ...
  // Export). Inverse default of openSections above: a key absent here means
  // OPEN, present+true means collapsed -- the groups hold the primary editing
  // controls, so they start expanded and the person folds whichever they
  // aren't using. Session-scoped.
            ...
            <div style={{ padding: "18px 20px", borderBottom: "1px solid #dde0e6", background: "#fff", display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                {showAddPagePreview && !bulkImportMode && (() => {
                  ...
                  // suggested page -- can still add Home (or any other core type) here.
                  // Both the list and the "all pages added" check now share this one
                  // filter, so they can't drift out of sync with each other.
                  const addablePages = ALL_PAGES.concat(ADDITIONAL_PAGE_TYPES).filter(p =>
                    !selectedPages.includes(p.id) &&
                    !selectedPages.some(sid => sid.startsWith(p.id + "-")) &&
                    !customPages.find(cp => cp.id === p.id || cp.id.startsWith(p.id + "-"))
                  );
                  return (
